...
const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  ...
  return (
    <section id={SectionId.HOME} className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden bg-slate-900 w-full">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        ...
        <div className="animate-fade-up delay-200 flex flex-col sm:flex-row items-center justify-center gap-4">
          <a 
            ...
            className="w-full sm:w-auto px-8 py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-full font-semibold text-lg transition-all shadow-lg shadow-blue-900/20 hover:-translate-y-0.5"
          >
            Get a Quote
          </a>
          <button 
            onClick={() => onNavigate('services')}
            className="w-full sm:w-auto px-8 py-3.5 bg-white/5 hover:bg-white/10 text-white border border-white/10 backdrop-blur-md rounded-full font-semibold text-lg transition-all hover:-translate-y-0.5"
          >
            View Services
          </button>
        </div>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 pointer-events-none animate-fade-up delay-300">
        <div className="relative flex items-center justify-center">
            {/* Rotating Text Ring */}
            {/* We apply a base rotation from scroll (offset) + a continuous slow spin animation on the child */}
            <div 
                className="w-24 h-24 duration-75 ease-out origin-center"
                style={{ 
